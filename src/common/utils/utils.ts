...
export function calcuateDiscounts(value: number, percentage: number): number {
  return Number(value) - Number(value) * (Number(percentage) / 100);
}

export async function uploadToStorageBuffer(
  file: any,
...
export const generateToken = (
  userId: string,
  expires: number,
  secret: string = process.env.JWT_SECRET || 'jwtsecret',
): string => {
  const payload = {
    sub: userId,
...
export const getAmbientURL = (ambient: string): string => {
  if (process.env.NODE_ENV === 'development')
    return `http://${ambient}.localhost:3000`;
...
export function calculateDiscount(value: number, percentage: number): number {
  return Number(value) - Number(value) * (Number(percentage) / 100);
}

export function getOnlyProfessionals(information: any[]): any[] {
  if (!information.length) return [];

  const mapped = information.map((i) => ({
    value: i.id_profissional,
    label: i.nome_profissional,
  }));

  return Array.from(new Set(mapped.map((obj) => JSON.stringify(obj)))).map(
...
export function getFilteredData(filters: object = {}, values: any[]): any[] {
  const isEmptyObject = isObjectEmpty(filters);
  if (isEmptyObject) return values;
  const filter = Object.values(filters).join(',');

  return values.filter((value) =>
    JSON.stringify(value).toLowerCase().includes(String(filter).toLowerCase()),
  );
}
