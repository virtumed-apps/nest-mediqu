...
async function pagamento({
  ...
  const CHAVE_PESSOA =
    process.env[`${id_ambiente}_CONTSELF_CHAVE_PESSOA`.toUpperCase()];
  ...
  const conselhorProfissional = await agendamentoService.getConselho(
    profissional.conselho,
  );

  if (!configs || !configs.payment)
    return { erro: 'Nenhuma configuração de ambiente sobre pagamento' };

  const split = [];

  configs.payment.split.medic > 0 &&
    profissional.cnpj &&
    split.push({
  ...
  configs.payment.split.company > 0 &&
    split.push({
      EmiteNotaOurecibo: !!configs.payment.emitirNotaFiscal ? 1 : 0,
      Identificador: configs.payment.CNPJ,
      descricaoServico: `REFERENTE A - ${
        !!profissional.descricaoServico
          ? profissional.descricaoServico
          : 'Consulta'
      } com ${profissional.titulo} ${profissional.nome} - ${
        agendamento.especialidade
      } - ${conselhorProfissional.nome}-${profissional.uf_conselho} ${
        profissional.numero_conselho
      } em ${moment(agendamento.data_hora).format('DD/MM/YY HH:mm')}`,
      ValorDivisao: !!profissional.cnpj
        ? (valor * configs.payment.split.company) / 100
        : (valor *
            (configs.payment.split.company + configs.payment.split.medic)) /
          100,
    });

  configs.payment.split.intermediary > 0 &&
    split.push({
      EmiteNotaOurecibo: !!configs.payment.emitirNotaFiscal ? 1 : 0,
      Identificador: `REFERENTE A - ${
        !!profissional.descricaoServico
          ? profissional.descricaoServico
          : 'Consulta'
      } com ${profissional.titulo} ${profissional.nome} - ${
  ...
  const body = {
    ...
    Telefone:
      removeSpecialCharacters(paciente.celular || paciente.telefone) || null,
    TokenizaCartao: 0,
    UF: paciente.uf || null,
    Valor: valor || 0,
    // Valor: 1,
  ...
  const CONFIG = {
    ...
    headers: {
      Authorization: `Basic ${TOKEN_PAGAMENTO}`,
